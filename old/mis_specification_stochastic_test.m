...
% kappa_now= @(x) (x<a_true).*ones(size(x))+(x>=a_true).*b_true.*ones(size(x));
A=FEM1D_stiff(hg,kappa_now);

% v=laplace_mtx(t*hg)'; 
% A_lap=spdiags([-v(2:N+1),v(1:N)+v(2:N+1),-v(1:N)],-1:1,N,N)/hg^2;
kappa_now= @(x) laplace_mtx(x);
A_lap=FEM1D_stiff(hg,kappa_now);
...
Du=zeros(1,Nh-1);
for i=1:Nh
    D(i)=integral(kappa,(i-1)*h,(i+1)*h,'AbsTol',1e-2)/h^2;
end
for i=1:Nh-1
    Du(i)=-integral(kappa,i*h,(i+1)*h,'AbsTol',1e-2)/h^2;
end
