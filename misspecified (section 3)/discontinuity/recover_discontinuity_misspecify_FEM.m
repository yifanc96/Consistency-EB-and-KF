...
% the covariance of the rhs
[eigvec,eigval]=eig(full(A_lap));
s=1; % true regularity of the rhs
...
Du=zeros(1,Nh-1);
for i=1:Nh
    D(i)=integral(kappa,(i-1)*h,(i+1)*h,'AbsTol',1e-5)/h^2;
end
for i=1:Nh-1
    Du(i)=-integral(kappa,i*h,(i+1)*h,'AbsTol',1e-5)/h^2;
end
