...
rhs=[zeros(N/2-1,1);1;zeros(N/2,1)]/hg;
G=eigvec*diag(1./diag(eigval).^s)*eigvec';
u=G*rhs; % generate samples of GP

s_search=0.5:0.02:3; la=length(s_search); %grid search of s

array_q=8:8; lq=length(array_q); %n: num of data we observe; want to see the asymptotics when n goes to infinity
sol_MLE=zeros(lq,1); sol_KF=sol_MLE; % store estimators
testerr=zeros(lq,2);

L_MLE=zeros(la,lq); L_KF=zeros(la,lq); %loss function
tic
for iter=1:lq
    n=2^array_q(iter);  %num of data observed
    ...
    for iter_a=1:la
        a=s_search(iter_a);
        Theta_global=eigvec*diag(1./diag(eigval.^a))*eigvec'/hg;
